...
public class InAppWebViewChromeClient extends WebChromeClient implements PluginRegistry.ActivityResultListener, ActivityResultListener {
  ...
  public boolean onCreateWindow(WebView view, boolean isDialog, boolean isUserGesture, final Message resultMsg) {
    ...
    if(result.getType() == WebView.HitTestResult.UNKNOWN_TYPE) {
      WebView targetWebView = getTargetWebView(isDialog, isUserGesture, resultMsg);
  ...
  private WebView getTargetWebView(boolean isDialog, boolean isUserGesture, Message resultMsg) {
    if (getActivity() == null) {
      return null;
    }
